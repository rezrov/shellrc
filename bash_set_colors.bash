...
# Centralized color setup for interactive bash shells. Responsible for:
#  - LS_COLORS (GNU/uutils ls) via dircolors, with an embedded fallback for
#    systems without the dircolors binary.
#  - LSCOLORS (BSD ls, e.g., the default /bin/ls on macOS) so output is
#    colorized even on installs without GNU coreutils.
#  - A colored PS1 if the terminal advertises 8+ colors.
#  - FORCE_COLOR, for tools that respect it (chalk, jest, npm, etc.).
#
# The "ls --color=auto" and "grep --color=auto" aliases live in
# bash_aliases.bash, not here.

# Don't change these lines

# LS_COLORS: GNU/uutils ls reads this. The source of truth is
# ~/.shellrc/dircolors. The embedded LS_COLORS string in the else-branch is a
# fallback for systems without the dircolors binary; regenerate it with:
#   dircolors -b ~/.shellrc/dircolors
...
# LSCOLORS: BSD ls reads this (e.g., default /bin/ls on macOS). 11
# foreground/background letter pairs covering directories, symlinks, etc.
# See "man ls" on BSD/macOS. Lowercase = normal, uppercase = bold.
# This is the BSD default with bold blue for directories.
export LSCOLORS='Exfxcxdxbxegedabagacad'

# MODIFICATIONS BELOW THIS LINE

# PS1: colored if the terminal advertises 8+ colors, plain otherwise.
...
# Force color output from tools that respect this convention (chalk, jest,
# npm, etc.), even when they can't autodetect a TTY (e.g., in pipelines).
